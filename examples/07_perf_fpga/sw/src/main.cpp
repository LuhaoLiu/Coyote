...
double run_bench(
    ...
    for(int j = 0; j < n_runs; j++) {
        ...
        if (mapped) {
            coyote_thread->userMap(mem, size);
        }
        // Run the benchmark
...
int main(int argc, char *argv[]) {
    ...
    while(curr_size <= max_size) {
        ...
        if (n_throughput_reps != 0) {
            double throughput_time = run_bench(coyote_thread, curr_size, mem, n_throughput_reps, n_runs, oper, mapped);
        ...
        // Run latency test
        if (n_latency_reps != 0) {
            double latency_time = run_bench(coyote_thread, curr_size, mem, n_latency_reps, n_runs, oper, mapped);
